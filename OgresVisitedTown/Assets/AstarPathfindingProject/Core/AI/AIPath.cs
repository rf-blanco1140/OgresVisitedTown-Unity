...
namespace Pathfinding {
	...
	public partial class AIPath : AIBase, IAstarAI {
		...
		protected override void MovementUpdateInternal (float deltaTime, out Vector3 nextPosition, out Quaternion nextRotation) {
			...
			bool isWalking = true;
			if (nextPosition == currentPosition) { isWalking = false; }
			SetAnimatorWalkingValue(isWalking);
		}

		private void SetAnimatorWalkingValue(bool pValue)
        {
			myAnimator.SetBool("IsMoving", pValue);
        }
